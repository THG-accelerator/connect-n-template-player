...
public class LCABoardAnalyser {
    ...
    public LCABoardAnalyser(GameConfig config) {
    ...
    public LCAGameState calculateGameState(Board board) {
        List<LCALine> lines = getLines(board);
        Map<Counter, Integer> bestRunByColour = new HashMap<>();
        for (LCALine line : lines) {
        ...
        return new LCAGameState(bestRunByColour, board.getConfig(), boardFull);
    }
    ...
    private List<LCALine> getLines(Board board) {
        ArrayList<LCALine> lines = new ArrayList<>();
        for (Map.Entry<Function<Position, Position>, List<Position>> entry : positionsByFunction
            ...
            lines.addAll(startPositions.stream().map(p -> new LCABoardLine(board, p, function))
                    .collect(Collectors.toList()));
        }
        return lines;
    }

    private Map<Counter, Integer> getBestRunByColour(LCALine line) {
